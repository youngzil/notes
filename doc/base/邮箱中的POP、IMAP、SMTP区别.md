...
对于IMAP邮件系统来说，一封邮件有3个时间戳：信头里的发送时间，信头里的接收时间，和服务器上的接收时间(这个又称为internal-date)。前两个时间戳储存在邮件的信头中，最后一个时间戳(internal-date)储存在邮件服务器上。

这个神奇的internal-date表示了邮件进入服务器时的时间，通常情况下，这个internal-date和信头里的接收时间应该是一致的或非常接近。但在如下情况下这两个时间会有所不同：
我在web上误删了邮箱中的一些邮件，而我在电脑客户端里有这些邮件的备份，于是通过IMAP账号同步这些邮件，相当于把这些邮件“回传”给了服务器上。此时，信头里的接收时间依然是当初那个时间，但是internal-date已经变成我回传邮件的时间了。

对于foxmail和outlook等客户端，它们直接把信头里的接收时间当作邮件的接收时间来显示，而对于OS X下的原生Mail.app，它会向服务器索要internal-date，然后按服务器返回的internal-date来显示邮件。这会使得我的那些被回传的邮件不能显示正确的时间。




参考  
https://www.zhihu.com/question/28109015
